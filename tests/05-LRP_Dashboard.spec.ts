...
test(' Dashboard Received ', async ({ page , baseURL}) => {
   ...
   //Due Tomorrow Grid
    ...
    const openMatch = dashboardCountText === totalCount.toString();
    ...
    // Soft assertions: all four are evaluated and reported, and the test goes red
    // if any count disagrees — previously a mismatch only showed up in the email.
    expect.soft(openMatch, `Open: dashboard ${dashboardCountText} vs grids ${totalCount}`).toBe(true);
    expect.soft(overdueMatch, `Overdue: dashboard ${dashboardOverdue} vs grid ${OverdueGrid}`).toBe(true);
    expect.soft(atRiskMatch, `At Risk: dashboard ${dashboardRisk} vs grid ${AtRiskGrid}`).toBe(true);
