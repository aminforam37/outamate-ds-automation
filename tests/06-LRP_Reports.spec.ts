...
test(' Reports ', async ({ page, baseURL }) => {
    ...
    // Base folder
    const baseReportsDir = 'C:\\DS_playwright-test 1\\LRP_Reports';
...
//   const reportsDir2 = 'C:\\DS_playwright-test\\AOM_Reports';
 ...
 // Original downloaded file name
  ...
  // Save with SAME downloaded file name
  const uniqueFileNameRev1 = originalFileNameRev1.replace(extensionRev1, `_client${extensionRev1}`); //Added New
  const filePathRev1 = path.join(currentRunDir, uniqueFileNameRev1);
...
//     const reportsDirRev2 = 'C:\\DS_playwright-test\\AOM_Reports';
 ...
 // Original downloaded file name
  ...
  // Save with SAME downloaded file name
  const uniqueFileNameRev2 = originalFileNameRev2.replace(extensionRev2, `_order${extensionRev2}`);
  const filePathRev2 = path.join(currentRunDir, uniqueFileNameRev2);
 ...
 // Original downloaded file name
  ...
  // Save with SAME downloaded file name
  const uniqueFileNameFee = originalFileNameFee.replace(extensionFee, `_fee-type${extensionFee}`); //Added New
  const filePathFee = path.join(currentRunDir, uniqueFileNameFee); 
 ...
 // Original downloaded file name
  ...
  // Save with SAME downloaded file name
  const uniqueFileNameCOGS = originalFileNameCOGS.replace(extensionCOGS, `_cogs-type${extensionCOGS}`); //Added New
  const filePathCOGS = path.join(currentRunDir, uniqueFileNameCOGS);
 ...
 // Original downloaded file name
  ...
  console.log(`File downloaded successfully: ${originalFileNameQC}`);
   ...
   // console.log('File is valid and not empty');
         ...
         const parentFolder = 'C:\\DS_playwright-test 1\\LRP_Reports';
      ...
      //  console.log(`   Total files in folder: ${allFiles.length}`);
        ...
        // ---------- 1. New Orders Export Report (expect 1) ----------
        {
            ...
            addResult({
                ...
                status: 'Pass',
                URL: `<a href="${base}/app/internal/lien-release/reports/order-export">Order Export Report</a>`
            });
            } else {
        ...
        // ---------- 2. Pipeline Report (expect 1) ----------
        {
          ...
          //     console.log(`✅ Pipeline Report — found ${count} file`);
            ...
            addResult({
                ...
                status: 'Pass',
                URL: `<a href="${base}/app/internal/lien-release/reports/pipeline-report">Pipeline Report</a>`
            });
            } else {
        ...
        files.forEach((f, index) => { //console.log(`     • ${f}`);
            addResult({
                ...
                status: 'Pass',
                URL: `<a href="${base}/app/internal/lien-release/reports/revenue-report">Revenue Report</a>`
            });
        });
        ...
        // ---------- 4. COGS Report (expect 2) ----------
    {
       // const { count, files } = countFiles('cogs');
            const files = allFiles.filter((f) => f.includes('fee-cogs'));
        ...
        addResult({
         ...
         files.forEach((f, index) => { //console.log(`     • ${f}`);
            ...
            addResult({
                ...
                status: 'Pass',
                URL: `<a href="${base}/app/internal/lien-release/reports/fee-cogs-report">Fee & COGS Report</a>`
            });
          });  
            } else {
       ...
       //     console.log(`✅ Quality Check Report — found ${count} file`);
            ...
            addResult({
                ...
                status: 'Pass',
                URL: `<a href="${base}/app/internal/lien-release/reports/quality-check-report">Quality Check Report</a>`
            });
            } else {
